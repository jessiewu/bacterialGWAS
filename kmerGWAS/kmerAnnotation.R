...
# Sarah Earle and Chieh-Hsi Wu (2015)

## kmer annotation script
help=paste("","kmerAnnotation.R",
           "Usage: Rscript kmer_annotation.R -chisq_results -kmer_results -present_ctrl -present_case -prefix -blastdb1 -blastdb2 -ncbi_db -signif -nproc",
           "       E.g. Rscript kmer_annotation.R -chisq_results gwaskmer.chisqStat.txt -kmer_results gwaskmer.kmer.txt -present_ctrl gwaskmer.nPresentCtrl.txt -present_case gwaskmer.nPresentCase.txt -prefix annotationrun1 -blastdb1 db1 -blastdb2 db2 -ncbi_db genedb.txt -nproc 8 ",
           "",
           "Options:",
           "-chisq_results",
           ...
           "-blastdb2",
           " Second blast database to use to annotate kmers (more extensive search)",
           "-ncbi_db",
...
#######################################################################################	
annotate_kmers=function(splitseq, prefix, nproc, blastdb1, blastdb2, wh_pos, kmers_signif, sg2, present_ctrl, present_case, kmer_output_file,
                        blastnPath = NULL){
...
getCommandLineInputMatrix = function(args = NULL){
  ...
  if(length(which(regexpr("-",inputs[,1]) != 1)) > 0){
    stop("Argument names must start with '-'! E.g. -ncbiAnnot.")
  }
...
if((length(args)%%2)!=0 | length(args)==0) {
  cat(help,sep="\n")
  message(paste(c("Number of arguments: ", length(args)), collapse=""))
...
checkExistence(c(chisq_results,kmer_results,present_ctrl,present_case,ncbi_db))
checkDBExistence(blastdb1)
checkDBExistence(blastdb2)
...
topXChisq = as.numeric(scan(topXChisqPath, what=character(0), sep="\n"))
topXChisqOrder = order(topXChisq, decreasing=T)
wh = topXChisq[topXChisqOrder]
