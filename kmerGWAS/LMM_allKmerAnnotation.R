...
# for LMM analyses on all kmers.
# Chieh-Hsi Wu (2016)

## kmer annotation script
help=paste("",
           "LMM_allKmerAnnotation.R",
           "Usage: Rscript LMM_allKmerAnnotation.R -chisq_results -kmer_results -present_ctrl -present_case -prefix -blastdb1 -blastdb2 -ncbi_db -signif -nproc -LMM_kmers -LMM_output",
           "       E.g. Rscript LMM_allKmerAnnotation.R -chisq_results gwaskmer.chisqStat.txt -kmer_results gwaskmer.kmer.txt -present_ctrl gwaskmer.nPresentCtrl.txt -present_case gwaskmer.nPresentCase.txt -prefix annotationrun1 -blastdb1 db1 -blastdb2 db2 -ncbi_db genedb.txt -nproc 8 -LMM_kmers gwasLMM_kmers_used.txt -LMM_output gwasLMM_allkmers_out.txt",
           "",
           "Options:",
           "-chisq_results",
...
# Read in pattern index
LMM_kmers=scan(LMM_kmers, what=numeric(0), sep="\n")
# Danny's C++ code has indexes starting from 0 but R starts at 1
LMM_kmers=LMM_kmers+1

# Read in LMM output for all kmer patterns
LMM_output=read.table(LMM_output, header=T, sep="\t", quote="")

# Get -log10 Pvalues of kmers
sg=LMM_output$negLog10[LMM_kmers]

# Get the order of significance
od = order(sg, decreasing=TRUE)

# Index of the top x kmers
